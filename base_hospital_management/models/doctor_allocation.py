
from odoo import _, api, fields, models
from odoo.exceptions import ValidationError


class DoctorAllocation(models.Model):
    """Class holding doctor allocations"""
    _name = 'doctor.allocation'
    _description = 'Doctor Allocation'

    doctor_id = fields.Many2one('hr.employee', string="Doctor",
                                help='Name of the doctor',
                                domain=lambda self: [
                                    ('job_id.name', '=', 'Doctor'),
                                    ('company_id', '=', self.env.company.id),
                                ])
    name = fields.Char(string="Name", readonly=True, default='New',
                       help='Name of the allocation')
    department_id = fields.Many2one(string='Department',
                                    help='Department of the doctor',
                                    related='doctor_id.department_id')
    op_ids = fields.One2many('hospital.outpatient',
                             'doctor_id',
                             string='Booking',
                             help='Patient booking belongs to this '
                                  'allocation')
    company_id = fields.Many2one('res.company', string='Company',
                                 default=lambda self: self.env.company.id,
                                 help='Indicates the company')
    is_doctor = fields.Boolean(string='Is Doctor', help='True for doctors.')
    date = fields.Date(string='Date',
                       help='Indicate the allocated date for a doctor',
                       default=fields.Date.today())
    work_to = fields.Float(string='Work To', required=True,
                           help='Allocation time ending time in 24 hrs format')
    work_from = fields.Float(string='Work From', required=True,
                             help='Allocation time starting time in 24 hrs '
                                  'format')
    time_avg = fields.Float(string='Duration', help="Average Consultation time "
                                                    "per Patient in minutes",
                            readonly=False,
                            related='doctor_id.time_avg')
    patient_type = fields.Selection([('inpatient', 'Inpatient'),
                                     ('outpatient', 'Outpatient')],
                                    string='Patient Type',
                                    help='Indicates the type of patient')
    patient_limit = fields.Integer(string='Limit Patient',
                                   help='Maximum number of patients allowed '
                                        'per allocation', store=True,
                                   compute='_compute_patient_limit', )
    patient_count = fields.Integer(string='Patient Count',
                                   help='Number of patient under '
                                        'this allocation',
                                   compute='_compute_patient_count')
    slot_remaining = fields.Integer(string='Slots Remaining',
                                    help='Number of slots remaining in this'
                                         ' allocation', store=True,
                                    compute='_compute_slot_remaining')
    latest_slot = fields.Float(string='Available Slot',
                               help='Indicates the latest available slot')
    state = fields.Selection(
        [('draft', 'Draft'), ('confirm', 'Confirmed'),
         ('cancel', 'Cancelled')],
        default='draft', string='State', help='State of Doctor allocation')

    @api.model
    def _get_current_doctor_employee(self, company_id=False):
        """Find and repair the employee link for the logged-in doctor."""
        user = self.env.user
        company_id = company_id or self.env.company.id
        Employee = self.env['hr.employee'].sudo()
        doctor_domain = [
            ('company_id', '=', company_id),
            ('job_id.name', '=', 'Doctor'),
        ]

        doctor = Employee.search(
            doctor_domain + [('user_id', '=', user.id)], limit=1)
        if doctor:
            return doctor

        search_domains = []
        partner_field = 'address_home_id'
        if partner_field not in Employee._fields:
            partner_field = 'work_contact_id'
        if user.partner_id and partner_field in Employee._fields:
            search_domains.append([(partner_field, '=', user.partner_id.id)])
        if user.login:
            search_domains += [
                [('work_email', '=', user.login)],
                [('private_email', '=', user.login)],
            ]
        if user.name:
            search_domains.append([('name', '=ilike', user.name)])

        for domain in search_domains:
            doctor = Employee.search(doctor_domain + domain, limit=1)
            if doctor:
                if not doctor.user_id:
                    doctor.write({'user_id': user.id})
                return doctor

        return Employee

    @api.model
    def _auto_create_today_allocations(self, company):
        """Create today's confirmed allocations from the latest confirmed
        allocation for each doctor in the active company when missing.
        """
        today = fields.Date.today()
        doctors = self.env['hr.employee'].sudo().search([
            ('company_id', '=', company.id),
            ('job_id.name', '=', 'Doctor'),
        ])

        for doctor in doctors:
            today_allocation = self.sudo().search([
                ('company_id', '=', company.id),
                ('doctor_id', '=', doctor.id),
                ('date', '=', today),
                ('state', '=', 'confirm'),
                ('patient_type', 'in', [False, 'outpatient']),
            ], limit=1)
            if today_allocation:
                continue

            latest_allocation = self.sudo().search([
                ('company_id', '=', company.id),
                ('doctor_id', '=', doctor.id),
                ('state', '=', 'confirm'),
                ('patient_type', 'in', [False, 'outpatient']),
            ], order='date desc, id desc', limit=1)

            if latest_allocation:
                latest_allocation.copy({
                    'date': today,
                    'state': 'confirm',
                    'company_id': company.id,
                    'doctor_id': doctor.id,
                })

    @api.model
    def get_reception_allocations(self):
        """Return available outpatient allocations for the active company.

        If no allocation exists for today, create one from the doctor's last
        confirmed outpatient allocation so reception does not need manual
        daily re-entry.
        """
        company = self.env.company
        today = fields.Date.today()
        self._auto_create_today_allocations(company)

        allocations = self.sudo().search([
            ('company_id', '=', company.id),
            ('state', '=', 'confirm'),
            ('patient_type', 'in', [False, 'outpatient']),
            ('date', '>=', today),
        ], order='date asc, doctor_id asc, work_from asc')

        return allocations.read(['display_name'])

    @api.model
    def create(self, vals):
        """Method for creating name"""
        vals.setdefault('company_id', self.env.company.id)
        work_from_hr = int(vals.get('work_from', 0))
        work_from_min = int((vals.get('work_from', 0) - work_from_hr) * 60)
        work_from = "{:02d}:{:02d}".format(work_from_hr, work_from_min)
        work_to_hr = int(vals.get('work_to', 0))
        work_to_min = int((vals.get('work_to', 0) - work_to_hr) * 60)
        work_to = "{:02d}:{:02d}".format(work_to_hr, work_to_min)

        user = self.env.user
        is_doctor = user.has_group(
            'base_hospital_management.base_hospital_management_group_doctor')
        can_select_doctor = (
            user.has_group('base_hospital_management.base_hospital_management_group_manager')
            or user.has_group('base.group_system')
        )
        Employee = self.env['hr.employee'].sudo()

        doctor = Employee
        if is_doctor and not can_select_doctor:
            doctor = self._get_current_doctor_employee(vals.get('company_id'))
            if not doctor and vals.get('doctor_id'):
                selected_doctor = Employee.browse(vals['doctor_id']).exists()
                if selected_doctor.user_id and selected_doctor.user_id != user:
                    raise ValidationError(_(
                        "The selected doctor employee is already linked to "
                        "another user. Please choose your own employee record "
                        "or ask an administrator to update it."))
                if selected_doctor.company_id.id != vals.get('company_id'):
                    raise ValidationError(_(
                        "The selected doctor employee belongs to another "
                        "company. Please select a doctor from the active "
                        "company."))
                if selected_doctor.job_id.name != 'Doctor':
                    raise ValidationError(_(
                        "The selected employee is not configured with the "
                        "Doctor job position."))
                if not selected_doctor.user_id:
                    selected_doctor.write({'user_id': user.id})
                doctor = selected_doctor
        elif vals.get('doctor_id'):
            doctor = Employee.browse(vals['doctor_id']).exists()

        if not doctor:
            raise ValidationError(_("Please select a doctor employee."))

        vals['doctor_id'] = doctor.id
        vals['name'] = doctor.name + ': ' + work_from + '-' + work_to
        return super().create(vals)

    @api.onchange('work_from', 'work_to')
    def _onchange_work_from(self):
        """Method for calculating name"""
        if self.work_from and self.work_to:
            work_from_hr = int(self.work_from)
            work_from_min = int((self.work_from - work_from_hr) * 60)
            work_from = "{:02d}:{:02d}".format(work_from_hr, work_from_min)
            work_to_hr = int(self.work_to)
            work_to_min = int((self.work_to - work_to_hr) * 60)
            work_to = "{:02d}:{:02d}".format(work_to_hr, work_to_min)
            self.name = str(self.doctor_id.name) + ': ' + work_from + '-' + work_to

    @api.model
    def default_get(self, doctor_id):
        """Method for making doctor field readonly and applying default value
        for doctor login"""
        res = super().default_get(doctor_id)
        user = self.env.user
        is_doctor = user.has_group(
            'base_hospital_management.base_hospital_management_group_doctor')
        can_select_doctor = (
            user.has_group('base_hospital_management.base_hospital_management_group_manager')
            or user.has_group('base.group_system')
        )
        if is_doctor and not can_select_doctor:
            default_doctor_id = self._get_current_doctor_employee()
            if default_doctor_id:
                res['doctor_id'] = default_doctor_id.id
                res['is_doctor'] = True
                self.is_doctor = True
        else:
            self.is_doctor = False
            res['is_doctor'] = False
        return res

    @api.constrains('doctor_id', 'company_id', 'work_from', 'work_to', 'date')
    def _check_overlap(self):
        """Method for checking overlapping"""
        for allocation in self:
            if allocation.work_from >= allocation.work_to:
                raise ValidationError("Work From must be less than Work To.")

    @api.depends('work_from', 'work_to', 'time_avg')
    def _compute_patient_limit(self):
        """Method for computing patient limit"""
        for record in self:
            if (record.work_from and record.work_to and record.time_avg
                    and record.time_avg > 0):
                patient_slots = int(
                    (record.work_to - record.work_from) / record.time_avg)
                if patient_slots <= 0:
                    raise ValidationError(
                        "Work From must be less than Work To.")
                else:
                    record.patient_limit = patient_slots
            else:
                record.patient_limit = 10

    @api.depends('op_ids')
    def _compute_patient_count(self):
        """Method for computing patient count"""
        for rec in self:
            rec.patient_count = len(rec.op_ids)

    @api.depends('op_ids', 'patient_count', 'patient_limit')
    def _compute_slot_remaining(self):
        """Method for computing slot remaining"""
        for rec in self:
            rec.slot_remaining = rec.patient_limit - rec.patient_count

    def action_get_patient_booking(self):
        """Returns form view of bed"""
        return {
            'name': "Patient Booking",
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'res_model': 'hospital.outpatient',
            'domain': [('id', 'in', self.op_ids.ids)],
            'context': {'create': False}
        }

    def action_confirm_allocation(self):
        """Confirmation of allocation"""
        self.state = 'confirm'

    def action_cancel_allocation(self):
        """Method for cancelling a allocation"""
        self.state = 'cancel'

    @api.model
    def get_allocation_lines(self):
        """Returns allocated hour details to display on the dashboard"""
        data_list = []
        allocated_hour = self.sudo().search([
            ('doctor_id.user_id', '=', self.env.user.id),
            ('company_id', '=', self.env.company.id),
        ])
        for rec in allocated_hour:
            data_list.append({
                'date': rec.date,
                'name': rec.name,
                'patient_type': rec.patient_type,
                'patient_limit': rec.patient_limit,
                'patient_count': rec.patient_count
            })
        return data_list
