...
class HospitalInpatient(models.Model):
        ...
        if self.enable_outpatient:
            ...
            self.sudo().write({
                'prescription_ids': [(0, 0, {
                    ...
                    'note': rec.note,
                    'selected_times': [(6, 0, rec.selected_times.ids)],
                    'days': rec.days,
    ...
    def action_invoice(self):
        ...
        for rec in self:
            if rec.bed_rent_amount:
                # Change this to use bed_rent (daily rate) instead of bed_rent_amount (total)
                inv_line_list.append((0, 0, {
                    'name': 'Room/Bed Rent Amount',
                    'price_unit': rec.bed_rent,  # Daily rate
                    'quantity': rec.admit_days,  # Number of days
                }))
            elif rec.room_rent_amount:
                # Similarly for room rent
                inv_line_list.append((0, 0, {
                    'name': 'Room/Bed Rent Amount',
                    'price_unit': rec.room_rent,  # Daily rate
                    'quantity': rec.admit_days,  # Number of days
                }))
    ...
    def action_print_prescription(self):
        ...
        for rec in self.prescription_ids:
            ...
            data = {
                'datas': p_list,
                'date': self.hosp_date or fields.Date.today(),
